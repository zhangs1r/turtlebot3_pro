...
"""NodeManager 维护 belief 地图上的可导航图：
- 以 NODE_RESOLUTION 在局部区域生成/更新节点
- 维护节点 utility、visited、neighbor 关系
- 提供 Dijkstra / A* 路径搜索工具
"""


class NodeManager:
    # 在 belief 地图上维护探索图；
    # 采用 QuadTree 存储节点，做坐标查找更快。
    ...
    def Dijkstra(self, start, boundary=None):
        ...
        while len(q) > 0:
            ...
            # 调试可打开断言：assert self.nodes_dict.find(u) is not None
    ...
    def h(self, coords_1, coords_2):
        # 备选启发式（曼哈顿距离）：
        # 曼哈顿示例：h = abs(coords_1[0] - coords_2[0]) + abs(coords_1[1] - coords_2[1])
        # 备选启发式（欧氏距离手写版）：
        # 欧氏示例：h = ((coords_1[0] - coords_2[0]) ** 2 + (coords_1[1] - coords_2[1]) ** 2) ** (1 / 2)
...
class Node:
    # 图节点包含三类核心信息：
    # - utility：该点可“消除”的 frontier 数量（探索价值）
    # - neighbor_set：局部可通行邻接关系
    # - visited：是否访问过（策略输入中的引导特征）
    ...
    def initialize_observable_frontiers(self, frontiers, updating_map_info):
        # 初始化该节点可观测到的 frontier 集合，数量即 utility。
        if len(frontiers) == 0:
            self.utility = 0
            return set()
        else:
    ...
    def update_neighbor_nodes(self, updating_map_info, nodes_dict):
        # 在 5x5 的 NODE_RESOLUTION 邻域内尝试建立连边。
            ...
            for j in range(self.neighbor_matrix.shape[1]):
                if self.neighbor_matrix[i, j] != -1:
                    ...
                    if neighbor_node is None:
                        # 调试候选逻辑（默认关闭）：
                        # 示例：cell = get_cell_position_from_coords(neighbor_coords, updating_map_info)
                        # 示例：if updating_map_info.map[cell[1], cell[0]] == 1:
                        # 示例：    self.neighbor_matrix[i, j] = 1
                        continue
                    else:
