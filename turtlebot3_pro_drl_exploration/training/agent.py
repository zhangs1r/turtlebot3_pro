...
"""Agent 连接“地图状态”和“神经网络策略”：
1. 接收 Env 的 belief map 与机器人位置。
2. 通过 NodeManager 维护局部可导航图。
3. 把图结构编码为固定形状张量，喂给 PolicyNet。
"""


class Agent:
    # Agent 是地图状态与神经策略之间的规划接口：
    # map -> 图结构 -> 张量观测 -> 采样下一航点。
    ...
    def update_observation(self):
        # 把 NodeManager 的动态图结构整理成稠密数组：
        # 节点坐标、节点效用、邻接矩阵、当前节点索引。
        all_node_coords = []
    ...
    def get_observation(self):
        # 把图数组打包为 model.py 期望的定长张量输入。
        # 每个节点特征为：[relative_x, relative_y, utility, visited]。
        ...
        # node_inputs: [1, N, 4]，后续会 pad 到 [1, NODE_PADDING_SIZE, 4]。
        ...
        # current_index: [1,1,1]，表示“当前节点在节点表中的索引”。
        ...
        # edge_padding_mask 中：
        # - 0: 可选真实动作
        # - 1: padding 或被屏蔽动作（pointer attention 会 mask 掉）
    ...
    def select_next_waypoint(self, observation):
        ...
        # 从策略分布中采样动作（不是 argmax），与 SAC 训练方式一致。
